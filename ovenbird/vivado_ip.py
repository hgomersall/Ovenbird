...
class VivadoIP(object):
    ...
    def get_vhdl_instance(self):
        ...
        a unique instance in HDL. The ``HDLCodeWthIP`` block looks and acts 
        ...
        suitable VHDL signal when the top level VHDL file is created.
    ...
    def write_verilog_wrapper(self, output_directory):
        '''Generates the verilog file that wraps the IP block, wrapping the 
        ports to convert between the myhdl types and names and the Vivado 
        expected types and names.
